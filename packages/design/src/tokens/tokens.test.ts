...
describe('space and radius', () => {
  ...
  it('names every radius after what wears it', () => {
    ...
    //
    // Six of these share a value with a neighbour, and the rule this test
    ...
    // and `glyph` is the same five pixels under the Order-tracking legend's
    // 20px specimen tiles. A shared value is fine; a shared name would not
    // be.
    //
    // `meterSeg` (1px) arrived with the same board, under its six-segment
    // dot meter. The assertion is a whole-list one on purpose — it is how a
    // radius gets added deliberately rather than by someone reaching for the
    // nearest one — so adding a step means coming here and saying what
    // wears it.
    expect(Object.keys(radius)).toEqual([
      'meterSeg', 'barWaterfall', 'barSmall', 'bubbleTail', 'barTop', 'checkbox',
      'glyph', 'checkboxLarge', 'chipSquare', 'segment', 'iconChip', 'button',
  ...
  it('orders the radii from chip to frame', () => {
    ...
    // with `barSmall`, `checkbox` with `barTop` and `glyph` with
    // `checkboxLarge`, and a tie is not an order. No pair is ever adjacent
    // or compared, so nothing is lost.
    const px = [
      'meterSeg', 'barWaterfall', 'barSmall', 'barTop', 'checkboxLarge', 'chipSquare',
