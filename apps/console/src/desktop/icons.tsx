...
export type IconName =
  ...
  | 'coil'
  | 'bin'
  /* The Order-tracking board's moves. `lock` is above, with Messages': one
     padlock in the app, and the two geometries were two pixels apart. */
  | 'van'
  | 'doc'
  | 'tick'
  | 'move-on'
...
const P: Record<IconName, ReactElement> = {
  ...
  /**
   * The Order-tracking board's vocabulary. Its padlock is the one Messages
   * already draws, above: the frame's own is two pixels squatter, which at
   * thirteen pixels is nothing, and one padlock in the app beats two
   * near-identical ones under two names.
   *
